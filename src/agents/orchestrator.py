...
-> tool call -> MCP server -> engine update -> result back. The decision
step (`policy_stub`) and the message text are placeholders — no LLM call
happens yet. Phase 3 swaps in real strategy, Phase 4 swaps in LLM-authored
NL messages and belief updates; this module's turn loop is what they'll
plug into.
"""
...
from src.agents.policy_stub import candidate_actions, compose_message
...
async def _act(client: Client, session: GameSession, agent: str, barriers_remaining: int, rng: random.Random) -> dict:
    """Try candidate actions via `choose_action` until one is accepted."""
    for action in candidate_actions(agent, barriers_remaining, rng):
...
async def run_subgame_via_mcp(grid_size: tuple, max_moves: int, max_barriers: int,
                               cop_pos: tuple, thief_pos: tuple, rng: random.Random) -> dict:
    ...
    async with Client(cop_server) as cop_client, Client(thief_server) as thief_client:
        for round_number in range(1, max_moves + 1):
            thief_turn = await _act(thief_client, session, "thief", 0, rng)
            ...
            cop_turn = await _act(cop_client, session, "cop", barriers_remaining, rng)
...
async def run_series_via_mcp(grid_size: tuple, max_moves: int, num_games: int, max_barriers: int,
                              scoring: dict, start_positions_fn, seed: int | None = None) -> dict:
    ...
    for _ in range(num_games):
        ...
        result = await run_subgame_via_mcp(grid_size, max_moves, max_barriers, cop_pos, thief_pos, rng)
